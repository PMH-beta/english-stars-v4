...
**Android Zurück-Button** (`ui.js`): Back-Layer, **sofort beim Laden** aktiv (`initBackNav` via DOMContentLoaded — bewusst NICHT an `menu-screen`/Login gekoppelt). Setzt einen History-Wächter (`pushState` mit **gleicher URL** → Recovery-Hash unberührt) und einen `popstate`-Listener; zusätzlich `visibilitychange`/`pageshow` für die Rückkehr aus anderen Apps und `pointerdown`/`click`/`touchstart` (capture; `touchstart` passive) für die geste-gedeckte Auffrischung. `showScreen` merkt nur `_currentScreen`. **Grundregel: es liegt möglichst immer genau ein Wächter** — Re-Arming passiert **gestengedeckt**: im `popstate` (Folge des Back-Drucks) und bei jeder Nutzer-Geste (`_refreshGuardOnGesture` → `_armGuard` nur wenn `_guardPresent()` false, via `history.state.esBackGuard` als Quelle der Wahrheit). Grund: Chrome **überspringt gestenlos erzeugte History-Einträge beim Back** (Anti-Trap-Intervention) — daher scheitern Lade-Wächter, Timer und `_onAppResume` gestenlos; verlässlich wird der Wächter erst beim nächsten Tap. **Best-effort: 100% ist auf reiner PWA nicht erreichbar** (Chrome-Schutz); für garantierte Kontrolle wäre ein TWA-Wrapper nötig. Ob die App verlassen wird, entscheidet allein das Flag `_exitArmed`, nicht das Fehlen eines Wächters. `popstate` (= Zurück) in Priorität: (1) `_exitArmed` (zweiter Zurück im Exit-Fenster) → App verlassen via **einzelnem `history.back()`** (kein `go(-2)`); (2) oberstes Overlay schließen — generisch über die Dialog-Kennung `position:fixed`+`z-index:9999` bzw. `.es-overlay`; (3) `game-screen` → `confirmHome` (Speichern-Nachfrage); (4) sonst nicht im Menü (und kein Pre-Login-Screen aus `NAV_IGNORE`) → `showMenu`; (5) im Menü/Pre-Login → **Double-Back-to-Exit**: erster Zurück zeigt einen Toast (`z-index:10000`, damit `_topOverlay` ihn nicht als Modal greift), setzt `_exitArmed=true` und armt **sofort** neu, dazu ein Zeitfenster (~2,5 s, `_exitTimer`). Verstreicht das Fenster ohne zweiten Zurück, löscht der Timer **nur das Flag** (Wächter bleibt liegen) → nächster Zurück zeigt erneut den Toast. In (2)–(4) wird der Wächter synchron neu gesetzt. `_onAppResume` (Rückkehr sichtbar/pageshow) setzt `_exitArmed=false`, blendet den Toast aus und legt den Wächter neu, falls `history.state` keinen ausweist — selbst wenn der gestenlose `pushState` dort verschluckt würde, sorgt `_exitArmed=false` dafür, dass der nächste Zurück den Toast zeigt statt zu verlassen.
