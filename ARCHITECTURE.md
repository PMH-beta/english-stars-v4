...
| `speech.js` | TTS (Web Speech API) + Spracherkennung (Vosk offline) | `_initTTS`, `primeTTS`, `warmTTS`, `speakWord`, `speakWordOnce`, `warmAudio`, `warmIosMic`, `ensureMicStream`, `releaseMicStream`, `startVoskRecognition`, `startRecording`, `voskStop`, `stopVisualizer` |
...
**iOS-Audio-Eigenheiten:** Der physische Stumm-Schalter dämpft Web-Audio (TTS/SFX/Musik), solange die Audio-Session in „Ambient" ist. Der **Start-Button** setzt deshalb einmalig `navigator.audioSession.type = 'play-and-record'` (Web Audio Session API, Safari 16.4+) — Kategorie, die Aufnahme erlaubt **und** Ausgabe über den Stumm-Schalter spielt → Sounds/TTS auch lautlos in JEDEM Modus, **ohne** Mic-Prompt am Start. **Niemals** auf `'playback'` umschalten: das verbietet die Aufnahme → Vosk/Erkennung tot (genau dieser Toggle-Versuch wurde schon einmal revertet). Der Mic-Prompt kommt erst im Aussprache-Modus (`warmIosMic()` in `_launchGame`, wärmt zugleich Permission/Session vor Runde 1, sonst Visualizer erst ab Runde 2). Alles iOS-only (`navigator.audioSession` fehlt anderswo → no-op). Risiko: `play-and-record` kann Audio aufs Ohrhörer/leise routen — bei Regress die eine Zeile raus.
