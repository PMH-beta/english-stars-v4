...
**Android Zurück-Button** (`ui.js`): Back-Layer, **sofort beim Laden** aktiv (`initBackNav` via DOMContentLoaded — bewusst NICHT an `menu-screen`/Login gekoppelt). Setzt History-Wächter (`pushState` mit **gleicher URL** → Recovery-Hash unberührt) und einen `popstate`-Listener; zusätzlich `visibilitychange` (App-Wechsel), `pageshow` (bfcache) und `pointerdown`/`click`/`touchstart` (capture, Geste). `showScreen` merkt nur `_currentScreen`. **Grundregel: der volle Stand ist IMMER zwei Wächter-Einträge über dem Ausgang** (`_depth`/`_GUARD_DEPTH=2`, wiederhergestellt via `_ensureTwoGuards`). Diagnose hat gezeigt: `pushState` greift auch **ohne** Nutzer-Geste — das Problem war nicht das Verschlucken, sondern die **Tiefe**: bei nur einem Eintrag popt der erste Back ihn weg und der zweite fällt durch und verlässt. Mit zwei Einträgen landet der erste (Menü-)Back auf einem Wächter (Toast). **Nach JEDEM abgefangenen Ereignis wird voll auf 2 aufgefüllt** — abgefangener `popstate` inkl. Toast/`_exitArmed`, Timer-Ablauf, Geste (`_refreshGuardOnGesture`), App-Wechsel (`_onAppResume`) und `_onPageShow` (bei `event.persisted` zuvor `_depth=0` für kompletten Neuaufbau, sonst Top-up). So fällt ein einzelner Stray-Back nie durch. **Best-effort: 100% ist auf reiner PWA nicht garantiert** (Chrome-Schutz); für garantierte Kontrolle wäre ein TWA-Wrapper nötig. `popstate` (= Zurück) in Priorität (Tiefe wird bei jedem Pop um 1 verringert): (1) `_exitArmed` (zweiter Zurück im Exit-Fenster) → **bewusster Exit, EINZIGER Moment ohne Auffüllen**: durchpoppen lassen, `history.back()` schiebt zum Verlassen nach (kein `go(-2)`); (2) oberstes Overlay schließen — generisch über die Dialog-Kennung `position:fixed`+`z-index:9999` bzw. `.es-overlay` — dann `_ensureTwoGuards`; (3) `game-screen` → `confirmHome` (Speichern-Nachfrage) → `_ensureTwoGuards`; (4) sonst nicht im Menü (und kein Pre-Login-Screen aus `NAV_IGNORE`) → `showMenu` → `_ensureTwoGuards`; (5) im Menü/Pre-Login → **Double-Back-to-Exit**: Toast (`z-index:10000`, damit `_topOverlay` ihn nicht als Modal greift), `_exitArmed=true`, **sofort `_ensureTwoGuards`** + Zeitfenster (~2,5 s, `_exitTimer`); verstreicht das Fenster, löscht der Timer das Flag und füllt erneut auf 2.
