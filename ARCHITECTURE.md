...
**Android Zurück-Button** (`ui.js`): Back-Layer, **sofort beim Laden** aktiv (`initBackNav` via DOMContentLoaded — bewusst NICHT an `menu-screen`/Login gekoppelt, sonst gibt es Zustände ohne Wächter und Zurück verlässt die PWA). Setzt einen History-Wächter (`pushState` mit **gleicher URL** → Recovery-Hash unberührt) und einen `popstate`-Listener. `showScreen` merkt nur `_currentScreen`. `popstate` (= Zurück) in Priorität: (1) Schließen-Popup offen → Abbrechen; (2) oberstes Overlay schließen — generisch über die Dialog-Kennung `position:fixed`+`z-index:9999` bzw. `.es-overlay`; (3) `game-screen` → `confirmHome` (Speichern-Nachfrage); (4) sonst nicht im Menü (und kein Pre-Login-Screen aus `NAV_IGNORE`) → `showMenu`; (5) im Menü/Pre-Login → „App schließen?"-Popup (Ja → `history.go(-2)` verlässt; Abbrechen → bleibt). Wächter wird nach **jedem** abgefangenen Zurück neu gesetzt; während des absichtlichen Verlassens schützt `_doExit` vor Eigen-`popstate`.
