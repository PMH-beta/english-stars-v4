...
**iOS-Audio-Eigenheiten:** (1) Der physische Stumm-Schalter dämpft Web-Audio (TTS/SFX/Musik), bis eine Mic-`getUserMedia` die Session auf „PlayAndRecord" hebt — d. h. bei Stummschaltung kommt Ton erst nach einer Aussprache-Runde. Erwartetes iOS-Verhalten, nicht per Code behebbar. (2) Die Mic-Permission wird pro Session neu abgefragt — auch nicht vermeidbar. `warmIosMic()` (aus `_launchGame` bei Pronounce-Modi) zieht den Prompt + die Session-Etablierung auf den Spielstart vor, statt mitten in Runde 1; sonst blockiert der Dialog Runde 1 und der Visualizer kommt erst ab Runde 2.
