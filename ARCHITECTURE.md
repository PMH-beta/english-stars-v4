...
**Android Zurück-Button** (`ui.js`): generischer Screen-Verlaufsstapel. `showScreen` ist die einzige Navigations-Stelle → pflegt automatisch `_screenStack` (menu = Basis; bekannter Screen im Pfad → dorthin zurückkürzen, sonst push). Neue Screens laufen ohne Verkabelung mit, sofern über `showScreen` geöffnet. Der Back-Layer (`initBackNav`) wird **erst beim ersten Erreichen von `menu-screen`** aktiviert (nach `restoreLastScreen`, Pre-Login ausgenommen via `NAV_IGNORE`) und setzt einen History-Wächter (`pushState` mit **gleicher URL** → Recovery-Hash unberührt). `popstate` (= Zurück) arbeitet in Priorität: (1) oberstes Overlay schließen — generisch erkannt über die gemeinsame Dialog-Kennung `position:fixed`+`z-index:9999` bzw. `.es-overlay`; (2) Screen-eigene Aktion aus `BACK_OVERRIDES` (`game-screen`→`confirmHome` mit Speichern-Nachfrage, `end-screen`→`showMenu`); (3) generisch eine Ebene zurück; (4) auf der Startseite Double-Back-Toast „Nochmal Zurück zum Schließen" (~3 s), zweiter Druck verlässt die App. Der Wächter wird nach **jedem** abgefangenen Zurück neu gesetzt (auch wenn ein Override-Dialog abgebrochen wird).
