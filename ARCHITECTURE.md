...
**Android Zurück-Button** (`ui.js`): Back-Layer für **In-App-Back**, **sofort beim Laden** aktiv (`initBackNav` via DOMContentLoaded — bewusst NICHT an `menu-screen`/Login gekoppelt). Ein History-Wächter (`_armGuard` → `pushState` mit **gleicher URL** → Recovery-Hash unberührt; Marker `esBackGuard`) + `popstate`-Listener fangen den Zurück ab, solange ein Wächter liegt. `showScreen` merkt nur `_currentScreen`. Der Wächter wird **nur gestengedeckt** gesetzt (Chrome ignoriert gestenlos erzeugte Einträge beim Back): einmal beim Init, im `popstate` selbst (gestennah, nach abgefangenem In-App-Back) und bei echter Geste, wenn keiner liegt (`_refreshGuardOnGesture` via `pointerdown`/`click`/`touchstart` capture + `_guardPresent` aus `history.state`) — hält den In-App-Back nach App-Wechsel am Leben. `popstate` (= Zurück) in Priorität: (1) oberstes Overlay schließen — generisch über die Dialog-Kennung `position:fixed`+`z-index:9999` bzw. `.es-overlay` — dann `_armGuard`; (2) `game-screen` → `confirmHome` (Speichern-Nachfrage) → `_armGuard`; (3) sonst nicht im Menü (und kein Pre-Login-Screen aus `NAV_IGNORE`) → `showMenu` → `_armGuard`; (4) im Menü/Pre-Login → nur Hinweis-**Toast** (`_showExitToast`, `z-index:10000`, damit `_topOverlay` ihn nicht als Modal greift; blendet sich nach ~2 s selbst aus) und **KEIN Re-Arm** → es liegt kein Wächter mehr, der **nächste Zurück verlässt die PWA**. **Den finalen Exit fangen wir bewusst NICHT ab** — auf reiner PWA nicht zuverlässig (Chrome-Geste-Sperre, Plattformgrenze). Für garantierte Kontrolle über den Hardware-Back wäre ein **TWA-Wrapper** nötig (s. PROJEKT-STATUS To-do).
