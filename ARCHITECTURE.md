...
**iOS-Audio-Eigenheiten:** Der physische Stumm-Schalter dämpft Web-Audio (TTS/SFX/Musik), bis eine Mic-`getUserMedia` die Session auf „PlayAndRecord" hebt — die einzige Web-Möglichkeit, Ton trotz Stummschalter über den Lautsprecher zu spielen. Deshalb ruft der **Start-Button** `warmIosMic()` (fire-and-forget) → eine einmalige getUserMedia hebt die Session für die ganze Seiten-Session, Stream wird sofort freigegeben (kein Dauer-Mic-Symbol) → Ton ab Start in JEDEM Modus, auch lautlos. **Bewusster Trade-off:** dafür sieht jeder iOS-Nutzer die Mic-Abfrage beim Start (auf iOS pro Session ohnehin unvermeidbar). `warmIosMic()` läuft zusätzlich in `_launchGame` bei Pronounce-Modi, damit Permission/Session vor Runde 1 warm ist (sonst blockiert der Dialog Runde 1 → Visualizer erst ab Runde 2). iOS-only, sonst no-op.
