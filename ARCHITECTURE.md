...
**Android Zurück-Button** (`ui.js`): Back-Layer, **sofort beim Laden** aktiv (`initBackNav` via DOMContentLoaded — bewusst NICHT an `menu-screen`/Login gekoppelt, sonst gibt es Zustände ohne Wächter und Zurück verlässt die PWA). Setzt einen History-Wächter (`pushState` mit **gleicher URL** → Recovery-Hash unberührt) und einen `popstate`-Listener. `showScreen` merkt nur `_currentScreen`. `popstate` (= Zurück) in Priorität: (1) oberstes Overlay schließen — generisch über die Dialog-Kennung `position:fixed`+`z-index:9999` bzw. `.es-overlay`; (2) `game-screen` → `confirmHome` (Speichern-Nachfrage); (3) sonst nicht im Menü (und kein Pre-Login-Screen aus `NAV_IGNORE`) → `showMenu`; (4) im Menü/Pre-Login → **Double-Back-to-Exit**: erster Zurück zeigt einen Toast (`z-index:10000`, damit `_topOverlay` ihn nicht als Modal greift) und armt **nicht** sofort neu, sondern startet ein Zeitfenster (~2,5 s, `_exitArmed`/`_exitTimer`); solange kein Wächter liegt, verlässt ein zweiter Zurück die App von selbst (Ursprungs-Eintrag = unterster, der Back läuft natürlich durch — kein `history.go`/`back`, kein JS-Close); verstreicht das Fenster, wird re-armt und der Toast ausgeblendet. Wächter wird nach **jedem** abgefangenen Zurück neu gesetzt — **außer** beim ersten Menü-Back (dort hält das Fehlen des Wächters das Exit-Fenster offen).
