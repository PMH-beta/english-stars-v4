...
| `speech.js` | TTS (Web Speech API) + Spracherkennung (Vosk offline) | `_initTTS`, `primeTTS`, `warmTTS`, `speakWord`, `speakWordOnce`, `warmAudio`, `warmIosMic`, `primeAudioSession`, `ensureMicStream`, `releaseMicStream`, `startVoskRecognition`, `startRecording`, `voskStop`, `stopVisualizer` |
...
| `game.js` | Spielmechanik: Fragen, Punkte, Streak, Exam | `_sfx`, `primeSfx`, `playSfx` + zahlreiche `window.*` Game-State-Variablen |
...
**Persistenter AudioContext** (`speech.js`): EIN geteilter `_audioCtx` für Visualizer **und** Vosk, statt pro Aufnahme `new AudioContext()` + `close()`. Stop-Pfade (`stopVisualizer`, `voskStop`) trennen nur die Nodes und `suspend()`en den Context; `_ensureAudioCtx` `resume()`t ihn beim nächsten Start. `warmAudio()` (aus `_launchGame` in der User-Geste bei Pronounce-Modi) weckt ihn schon zum Spielstart → Visualizer/Erkennung sind ab Runde 1 warm. Hintergrund: close()+new pro Runde war auf Mobile teuer und startete `suspended` (Kaltstart-Symptom: Ausschlag/TTS erst nach paar Runden; auf Android floss ohne `resume()` keine Audio in `acceptWaveform`).

**Audio-Unlock in der Start-Geste** (`startup.js` Start-Button): Mobile-Browser sperren Audio bis zur ersten User-Geste. Der Start-Button entsperrt deshalb ALLE drei getrennten Audio-Systeme: TTS (`warmTTS()` — echtes Wort bei volume=0, **awaited** → ~2 s Stimm-Ladezeit liegen hinter dem Ladebildschirm statt beim ersten gesprochenen Wort), SFX (`primeSfx()` — `_sfxCtx.resume()` + stiller Blip, sonst kamen Sounds erst nach 1-2 Runden) und Musik (`startMusicSync()`). Der Visualizer/Vosk-Context (`_audioCtx`) wird separat in der Mode-Button-Geste via `warmAudio()` geweckt (s. o.).

**iOS-Audio-Eigenheiten:** Der physische Stumm-Schalter dämpft Web-Audio (TTS/SFX/Musik), bis die iOS-Audio-Session aus „Ambient" gehoben wird. Zwei Wege:
- **Primär — Web Audio Session API** (Safari 16.4+, `primeAudioSession('playback')` im Start-Button): setzt `navigator.audioSession.type='playback'` → Ton spielt über den Stumm-Schalter, **ohne Mikrofon, ohne Prompt**. Vor echten Aufnahmen `'play-and-record'` (in `warmIosMic` + iOS-Zweig von `startRecording`), danach `releaseMicStream` zurück auf `'playback'`. (`'playback'` ist nicht-mischend → pausiert fremde Hintergrund-Audio wie Spotify.)
- **Fallback — Mic-Flip** (älteres iOS ohne die API): `warmIosMic()` macht eine einmalige `getUserMedia` (hebt die Session, Stream sofort frei, kein Dauer-Mic-Symbol). Kostet dann den Mic-Prompt beim Start.

`warmIosMic()` läuft außerdem in `_launchGame` bei Pronounce-Modi, damit Permission/Session vor Runde 1 warm ist (sonst blockiert der Dialog Runde 1 → Visualizer erst ab Runde 2). Der Aussprache-Mic-Prompt selbst ist auf iOS pro Session unvermeidbar. Alles iOS-only, sonst no-op.
