...
**Android Zurück-Button** (`ui.js`): Back-Layer, **sofort beim Laden** aktiv (`initBackNav` via DOMContentLoaded — bewusst NICHT an `menu-screen`/Login gekoppelt). Setzt History-Wächter (`pushState` mit **gleicher URL** → Recovery-Hash unberührt) und einen `popstate`-Listener; zusätzlich `visibilitychange` (App-Wechsel), `pageshow` (bfcache) und `pointerdown`/`click`/`touchstart` (capture, Geste). `showScreen` merkt nur `_currentScreen`. **Ansatz (letzter PWA-Versuch): nicht gegen Chromes Geste-Sperre kämpfen.** Chrome ignoriert Wächter-Einträge, die OHNE frische Nutzergeste gesetzt wurden (Timer/pageshow/resume) — künstliches Am-Leben-Halten scheitert. Daher wird der Wächter (`_depth`/`_GUARD_DEPTH=2`, via `_ensureTwoGuards`) **nur gestengedeckt** gesetzt: einmal beim Init, bei echten Gesten (`_refreshGuardOnGesture`, nur wenn `_depth<2`) und **im `popstate` selbst** (gilt als gestennah). **KEIN gestenloses Re-Arm** mehr: `_onAppResume`/`_onPageShow` und der Toast-Timer füllen **nicht** auf. Der **Exit ist rein zeitbasiert** (`_lastBackTs`, `EXIT_WINDOW≈2 s`), unabhängig vom Wächter-Stand. `popstate` (= Zurück) in Priorität: (1) oberstes Overlay schließen — generisch über die Dialog-Kennung `position:fixed`+`z-index:9999` bzw. `.es-overlay` — dann `_ensureTwoGuards`; (2) `game-screen` → `confirmHome` (Speichern-Nachfrage) → `_ensureTwoGuards`; (3) sonst nicht im Menü (und kein Pre-Login-Screen aus `NAV_IGNORE`) → `showMenu` → `_ensureTwoGuards`; (4) im Menü/Pre-Login → **Double-Back-to-Exit (zeitbasiert)**: zweiter Zurück innerhalb `EXIT_WINDOW` → Exit (nicht auffüllen, `history.back()` schiebt nach); sonst erster Zurück → Toast (`z-index:10000`, damit `_topOverlay` ihn nicht als Modal greift), `_lastBackTs` setzen, gestennah `_ensureTwoGuards`; ein Timer blendet den Toast nach `EXIT_WINDOW` nur aus (kein Re-Arm). **Bewusst akzeptiert: nach langer Inaktivität ohne Geste kann der erste Zurück die PWA verlassen** (Plattformgrenze); solange das Kind tippt, liegt ein Wächter und der erste Zurück zeigt den Toast. Für garantierte Kontrolle wäre ein TWA-Wrapper nötig.
